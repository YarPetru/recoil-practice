...
// 1 - selectors automatically recompute
// 2 - you can get other selectors in current selectors
// 3 - in useRecoilState we can use selector as atom if selector is settable
// 3.1 if selector unresetable - use useRecoilValue instead of useRecoilState
// 4 - selector are reusable. we can export it in any component and use
// 5 - in one unit of a selector we have a get and a set logic to deriving data
...
const euroSelector = selector<number>({
    key: 'eur',
    get: ({get}) => {
        // --use get function to get usdValue from component
        let usd = get(usdAtom)

        const commissionEnabled = get(commissionEnabledAtom)
        if (commissionEnabled) {
            const comission = get(commissionAtom)
            usd = removeCommission(usd, comission)
        }

        return usd * exchangeRate
        // return get(usdAtom) * exchangeRate
    },
    set: ({set, get}, newEuroValue) => {
        ...
        if (commissionEnabled) {
            const comission = get(commissionAtom)
            newUsdValue = addCommission(newUsdValue, comission)
        }

        // use set to set newValue to the component(where to and what amount to)
...
export const Selectors = () => {
    ...
    // const eur = useRecoilValue(euroSelector) // after doing eur - settable value we need useRecoilState
